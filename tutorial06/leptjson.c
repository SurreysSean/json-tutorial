...
static int lept_parse_object(lept_context *c, lept_value *v)
{
    ...
    for (;;)
    {
        ...
        char *tempS;
        if ((ret = lept_parse_string_raw(c, &tempS, &m.klen)) != LEPT_PARSE_OK)
            break;
        memcpy(m.k = (const char *)malloc(m.klen + 1), tempS, m.klen);
        m.k[m.klen] = '\0';
